...
**M0（骨架）、M1（媒体库与扫描）、M2（元数据与刮削）已完成，均在真实容器 + 真实浏览器上实测验收。**
M2 还剩 GUI 的其余部分（详情页/海报墙、批量匹配、设置页里填 TMDB 凭据），
之后是 **M3 播放核心** —— v0.1 打算发在那里。逐项清单与验收记录见
...
| 人工匹配 | 条目编辑（字段锁定） | 搜索 |
|---|---|---|
| ![人工匹配](docs/images/manual-match.png) | ![条目编辑](docs/images/item-edit.png) | ![搜索](docs/images/search.png) |
