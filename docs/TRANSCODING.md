...
`ffprobe` 会按需读文件头甚至寻道到文件尾（MKV 的 cues、MP4 的 moov）。
在本地盘上这是毫秒级，但在**网络存储/网盘上就是秒级**：实测本环境下单文件探测要
几秒到十几秒，9 个文件的夹具批次跑了几十秒。因此：

- 探测必须放在后台队列里，不能内联在扫描流程中（否则扫描会被拖到不可用）；
- ffprobe 要加 `-probesize 10M -analyzeduration 10M`：
  既限制读入量，也避免 AVI 这种没有索引的容器为了算时长把整个文件读完；
- 单文件要设超时（当前 2 分钟），超时标记失败且**不重试**，否则会无休止重试。
