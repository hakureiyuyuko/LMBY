...
### 内封字体（mkv 附件）—— 特效字幕的字从哪来（M4，2026-09-21）

mkv 可以把字体当**附件流**塞进容器，字幕里 `Style` 的 Fontname 与 `\fn` 引用的就是它们。
libass 只认「字体文件」：不给就只能拿兜底字体画 —— 用户看到的是**特效标题/美术字糊成一团**
（实测：一条片子的 OP 片头用了 13 条重叠事件 + `FOT-NewCezanne ProN EB` + `方正大雅宋_GBK`，
两个字体都在容器里内封着，却一个都没给 libass）。

**做法**（`internal/api/fonts.go` + `web/src/pages/Player.tsx`）：

- `GET /api/v1/play/{sid}/fonts` → 列出这个文件内封的字体（名字/大小/地址），还没抽完先回 **202**；
- `GET /api/v1/play/{sid}/fonts/{n}` → 第 n 个字体的字节（`Cache-Control: private, max-age=7d`，
  同一个文件不会变，浏览器缓存住就只需传一次）；
- 前端把这批 URL 交给 subtitles-octopus 的 `fonts: [...]` —— worker 会把它们**无条件加载**进
  libass 的字体目录（等价于 mpv 的 `fontsdir`），由 **libass 自己按字体内部的家族名匹配**。
  这比按名字喂 `availableFonts` 稳：那个要求我们猜中「字幕里写的那个名字」，而字体内部名表里
  常常是 GBK 编码的记录（实测 `fc-scan` 打出来就是乱码）。

**抽取为什么一次做完、又为什么慢**：mkv 的附件通常在文件末尾，逐个抽会把整部片子读 N 遍
（实测 223MB 的片、6 个附件：只抽 1 个 23.2s，一条命令里堆 6 个 19.1s —— 同一份读盘）。
所以先 ffprobe 数一下附件数（只读容器头部，实测 0.12s），再一条 ffmpeg 一次抽完并缓存。

⚠️ 不能直接用 `-dump_attachment:t ""`（按容器里的文件名落地）：ffmpeg 认为非 ASCII 文件名
「不安全」而**整体失败**（实测 `Filename 方正大雅宋_GBk.TTF is unsafe`），而内封字体几乎都是
中文名。所以落盘用自己起的安全名（`0.ttf`、`1.ttf`……），原始中文名只出现在 JSON 里
（顺带把路径穿越也堵死了）。

**代价与取舍（实例测量值，不是项目常量）**：抽取要读一遍源文件，223MB 的片约 19s，
多 GB 的 remux 要几分钟。所以它是**后台任务 + 缓存**，不阻塞播放：首次播这部长片时字幕先用
兜底字体画，抽完（前端轮询到 200）会自动重建 libass 实例切到真字体。缓存放在分片目录下，
服务重启即失效（下次播放重新抽）。

**仍会退化的**：字幕引用但容器里**没有内封**的字体（这个片子里就有 6 个：`A-OTF Ryumin Pro EH-KL`、
`Arial`、`方正悠宋 GBK 509R`、`方正悠黑_GBK 512B`、`腾祥伯当行楷GB18030`、`@HOT-FKaisho Std B`）——
库里没有的东西没法还原，要补只能把字体放到服务端（后续可把 `<数据目录>/fonts/` 整目录也喂给 libass）。

**验收**：`verify-transcode.sh` 第 14.5 节 —— 抽样真查附件（库里没登记附件，只能 ffprobe 真看）、
断言清单条数与附件数一致、字体字节的头 4 字节是真字体 magic（`00010000`/`OTTO`/`ttcf`/`true`）、
序号非法/越界回 400。实测：worker 会把 6 个字体**全拉走**（服务端日志里能看到
`/fonts/0…5` 都是 200 且字节数对得上）。

**字幕这条线还没做的**：外挂字幕的 GB18030 编码探测、外挂字幕上传、
把服务端字体目录（`<数据目录>/fonts/`）也喂给 libass（补那些容器里没内封的字体）。
