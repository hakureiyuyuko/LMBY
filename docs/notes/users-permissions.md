...
>
> **验收现状**（2026-09-22 晚更新）：
> 1. 「不允许转码」的 VOD 路径：**已打通（2026-09-22 晚）** —— 真因**不是环境**，是脚本：
>    它拿**受限用户**去扫候选条目，而真需要转码的条目对受限用户只会返回 403
>    （权限判定在决策之前），响应里根本没有 `mode`，于是永远「挑不到」。现在改成：
>    SQL 挑候选（浏览器解不开的编码 / 10bit / HDR，`probe_state='ok'`）→ **管理员**
>    确认 `mode=transcode` → 停掉那一路会话 → 受限用户打同一条期望 403；
...
路由 `/settings/users` 与页面 `Users.tsx` 在 M7 就写好了，`verify-users.sh` 的接口断言
