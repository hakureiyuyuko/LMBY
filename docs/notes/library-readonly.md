...
- 库列表每行：`只读` 徽章 + 「设为只读 / 改为可写」按钮（带 tooltip 说明它干什么）；
  旁边还有 **「编辑」**（改库类型与根路径 —— 下次扫描生效，界面写明了
  「改类型不影响已入库条目的 kind」「移除一条根路径不会删已入库的条目」）。
- 库详情卡：只读时显示一条说明 + **叠加层占用**（`GET /libraries/{id}` 的 `overlay` 字段：
  `files` / `bytes`）。
- **设置首页的叠加层看板**：`GET /api/v1/overlay` 汇总各库占用（带库名）+ 总计，
  每行一个「清空叠加层」按钮（弹窗说明：只删数据目录里这个库的刮削产物，
  媒体目录与数据库一个字都不动）。

## 四、真跑验收

`scripts/dev/verify-library-readonly.sh`（在跑服务的机器上执行，**35 通过 / 0 失败**）：
...
5. **编辑库**：改类型 → `GET` 反映；`paths` 是**替换语义**（给两条就两条、再给一条就只剩一条）、
   已入库条目不受影响；非法类型 / 相对路径 / 不存在的路径 / 空数组都是 400；最后改回去；
6. **看板与清空**：`GET /overlay` 带 `total` 与库名、未登录 401；
   `DELETE /libraries/{id}/overlay` 后统计归零、磁盘上确实没了，**再取一次图又会写回来**；
7. 参数校验与鉴权（空请求体 / 改名到空串 400、未登录 PATCH 401）。

元数据快照那条路径是**单独手工验的**（要真刮一次，脚本里不写死）：
对一个只读库的集强制重刮，约 6 秒后
`/var/lib/lmby/overlay/1/110/meta.json` 出现，内容是这次刮到的简介 / 评分 / `tmdb` id。

单测：`internal/overlay/overlay_test.go`（9 个用例：只读才写、可写不写、kind 消毒、
快照、Stats、**AllStats**、**Clear**、闸门、条目不存在）。
实例数据（不是项目常量）：图片那份 12 KB；`1 个文件 / 12015 字节`。

## 五、还没做的（说清楚，别让人以为有了）

1. **写回媒体目录 —— 用户 2026-09-22 明确要求不做**（本来在 ROADMAP 里是「以后的可选能力」）。
   于是 overlay 的定位就是最终落点：只读库的刮削产物住在那儿，
   `AssertWritable` 继续留着当「以后真有人要做写回」时的唯一闸门。
   取而代之的是设置页的**清空按钮**（想重来就一键清掉，不必去翻磁盘）。
2. **overlay 的容量上限 / 自动清理**：现在只统计、不清理（它算数据）。
   真要限制得给「按库预留多少」这类配置，属于以后的事。
