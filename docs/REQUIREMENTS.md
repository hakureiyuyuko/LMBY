...
4. 人工匹配与条目编辑（**刚需**，不是可选）
5. Web GUI（**首页大屏轮播 + 推荐行**、库浏览/海报墙、剧集视图、条目编辑与字段锁定、搜索（含联想与分面）、**收藏与播放列表/合集**、人工匹配与批量操作、设置、任务状态、**明暗主题切换**）
...
- 搜索的范围（M6 补齐，见 `docs/notes/search.md`）：不只作品标题，**还包括演职员的名字**
  （人名接的是 nfo 落库的 `people` 表，不联网、不占 TMDB 配额）。界面上有三件事：
  **即时联想**（下拉，防抖 200ms，与结果同一排序）、**结果分面**（类型 / 媒体库 / 流派 / 人
  四档带命中数，每一维统计时关掉自己那一维的筛选）、**「人」这一档**
  （人名搜索 + 参演作品数，点人名 → 只看这个人的作品）。
...
> **实现状态（2026-09-22）**：源/频道/播放/外链出口/定时刷新/失效频道标记/界面全部已落地并真机验收
> （见 `docs/ROADMAP.md` 的 M5 与 `docs/notes/livetv.md`）；界面在「直播」页（`web/src/pages/LiveTV.tsx`）：
> 频道列表（分组/搜索/收藏/启用停用/排序/logo/失效标记）+ 就地起播 + 管理员的源与探测面板。
