...
- [ ] **响应式（手机可用）+ i18n（zh-CN / en-US）** —— 响应式已完成，i18n 在做
      - **响应式**：三个断点（≤1024 平板横屏 / ≤768 平板竖屏与手机 / ≤480 手机）；
        手机顶栏换两行、导航自己一行横滑（右端渐隐提示）、行卡片与海报更小；
        表格与搜索结果卡片不会再把页面顶宽
      - 验收 `responsive-ui-test.mjs` **35/35**：8 个页面 × 3 个宽度**都没有横向滚动** +
        手机上顶栏两行、导航项单行（没被压成竖排）、能滑到「设置」、海报墙 ≥2 列、
        分面标签独占一行、编辑页表格可横滑；截图 `docs/images/home-phone.png`、
        `poster-wall-phone.png`
