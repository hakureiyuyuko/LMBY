...
46. **「文件过小」这类启发式必须可配置，阈值要按真实数据定**：用户报「正常片源被过滤了」，
    查出来是 **28 秒的 1080p h264 短片只有 646 KB**（ffprobe 确认有效），
    而旧默认阈值 1 MiB，一整个目录 63 个文件被当垃圾跳过。
    修法：默认降到 64 KiB（只挡缩略图/残片）、新增 `[scan] min_file_size` 可调，
    并把**阈值本身**写进问题描述（「661835 字节 < 阈值 65536」+ 去哪改）——
    否则用户只看到「文件过小」四个字，完全无从下手。
    教训：凡是「我认为它太小/不对就丢掉」的规则，都得（a）可配置，（b）把判据告诉用户。
