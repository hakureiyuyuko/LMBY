...
- [ ] **响应式（手机可用）+ i18n（zh-CN / en-US）** —— 两半都已完成：
      ...
      - **i18n**：**基础设施 + 浏览/播放路径已翻**（顶栏、登录页、首页、搜索与联想、
        海报墙、我的列表、公共标签）；语言探测 + localStorage 持久化 + 顶栏/登录页选择器；
        **后端保持语言中立**（行标题按 key 在客户端写，推荐依据用 API 的 taste/sourceWorks/seedTitle 拼）；
        验收 `i18n-ui-test.mjs` **25/25**；工具 `i18n-coverage.mjs`（未包 t 的文案 + 用了 t 却没目录）；
        设计与坑见 `docs/notes/i18n.md`
      - 　— 已完成**看片路径**（首页 → 海报墙 → 详情 → 播放）的翻译；
        待继续：管理面（库管理/条目编辑/人工匹配/直播/设置/会话/个人中心）共 **486 条**，
        以及两处「服务端给的文字」（报错文案、播放理由正文）要配 message code（跟 M7 一起做）；
        覆盖率随时可查：`node scripts/dev/i18n-coverage.mjs`
